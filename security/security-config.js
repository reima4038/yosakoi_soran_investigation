...
const securityMiddleware = (app) => {
  ...
  app.use(
    "/api/auth/login",
    createRateLimit(15 * 60 * 1000, 5, "ログイン試行回数が上限に達しました")
  );
  app.use(
    "/api/auth/register",
    createRateLimit(60 * 60 * 1000, 3, "登録試行回数が上限に達しました")
  );
  app.use(
    "/api/",
    createRateLimit(15 * 60 * 1000, 100, "API呼び出し回数が上限に達しました")
  );
          ...
          attachments: [
            {
              ...
              fields: [
                ...
                {
                  title: "詳細",
                  value: JSON.stringify(logEntry.details),
                  short: false,
                },
