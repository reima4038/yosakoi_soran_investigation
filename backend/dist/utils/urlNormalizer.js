...
exports.YouTubeURLNormalizer = exports.YOUTUBE_URL_PATTERNS = exports.URLValidationErrorType = void 0;
var URLValidationErrorType;
(function (URLValidationErrorType) {
    URLValidationErrorType["INVALID_FORMAT"] = "INVALID_FORMAT";
    URLValidationErrorType["NOT_YOUTUBE"] = "NOT_YOUTUBE";
    URLValidationErrorType["MISSING_VIDEO_ID"] = "MISSING_VIDEO_ID";
    URLValidationErrorType["PRIVATE_VIDEO"] = "PRIVATE_VIDEO";
    URLValidationErrorType["VIDEO_NOT_FOUND"] = "VIDEO_NOT_FOUND";
    URLValidationErrorType["NETWORK_ERROR"] = "NETWORK_ERROR";
    URLValidationErrorType["DUPLICATE_VIDEO"] = "DUPLICATE_VIDEO";
})(URLValidationErrorType || (exports.URLValidationErrorType = URLValidationErrorType = {}));
/**
 * YouTube URLパターンの定義
 */
exports.YOUTUBE_URL_PATTERNS = [
...
class YouTubeURLNormalizer {
    ...
    static normalize(url) {
        if (!url || typeof url !== 'string') {
            throw this.createValidationError(URLValidationErrorType.INVALID_FORMAT, 'URLが指定されていません', 'YouTube URLを入力してください', 'https://www.youtube.com/watch?v=dQw4w9WgXcQ');
        }
        // 前後の空白を除去
        ...
        if (!extractionResult.videoId) {
            ...
            if (!this.isYouTubeURL(normalizedUrl)) {
                throw this.createValidationError(URLValidationErrorType.NOT_YOUTUBE, 'YouTube以外のURLは登録できません', 'YouTube（youtube.com または youtu.be）のURLを入力してください');
            }
            throw this.createValidationError(URLValidationErrorType.MISSING_VIDEO_ID, 'ビデオIDが見つかりません', '完全なYouTube URLを入力してください', 'https://www.youtube.com/watch?v=dQw4w9WgXcQ');
        }
        // 正規化されたURLを生成
     ...
     * プロトコルが不足している場合に補完する
     */
    static addProtocolIfMissing(url) {
        if (!/^https?:\/\//.test(url)) {
            return `https://${url}`;
        }
        return url;
    ...
    static extractVideoId(url) {
        for (const pattern of exports.YOUTUBE_URL_PATTERNS) {
     ...
     * タイムパラメータを秒数に変換する
     */
    static parseTimeParameter(timeParam) {
     ...
     * バリデーションエラーを作成する
     */
    static createValidationError(type, message, suggestion, example) {
...
exports.YouTubeURLNormalizer = YouTubeURLNormalizer;
