...
router.post('/', middleware_1.authenticateToken, async (req, res) => {
    try {
        const { videoId, shareId, evaluationId, sessionId, title, description, startTime, endTime, isHighlight = false, tags = [], isPublic = true } = req.body;
        // 必須フィールドの検証
...
router.get('/', middleware_1.authenticateToken, async (req, res) => {
    try {
        const { videoId, shareId, evaluationId, sessionId, isPublic, page = 1, limit = 10 } = req.query;
        ...
        let filteredLinks = timestampLinks.filter(link => {
            ...
            if (videoId && !link.videoId.equals(new mongoose_1.default.Types.ObjectId(videoId))) {
                return false;
            }
            if (shareId && (!link.shareId || !link.shareId.equals(new mongoose_1.default.Types.ObjectId(shareId)))) {
                return false;
            }
            if (evaluationId && (!link.evaluationId || !link.evaluationId.equals(new mongoose_1.default.Types.ObjectId(evaluationId)))) {
                return false;
            }
            if (sessionId && (!link.sessionId || !link.sessionId.equals(new mongoose_1.default.Types.ObjectId(sessionId)))) {
                return false;
            }
        ...
        // 動画情報を追加
        const linksWithVideo = await Promise.all(paginatedLinks.map(async (link) => {
            const video = await Video_1.Video.findById(link.videoId).select('title youtubeId thumbnailUrl');
            return {
                ...link,
                video,
...
router.get('/:token', middleware_1.optionalAuth, async (req, res) => {
    try {
        ...
        if (!link.isPublic && (!userId || !link.creatorId.equals(new mongoose_1.default.Types.ObjectId(userId)))) {
        ...
        // 動画情報を取得
        const video = await Video_1.Video.findById(link.videoId);
        if (!video) {
            return res.status(404).json({
                status: 'error',
...
router.put('/:id', middleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        // 作成者のみ更新可能
        if (!link.creatorId.equals(new mongoose_1.default.Types.ObjectId(req.user.userId))) {
            return res.status(403).json({
                status: 'error',
...
// ハイライト区間一覧取得
router.get('/video/:videoId/highlights', middleware_1.optionalAuth, async (req, res) => {
    try {
        ...
        const highlights = timestampLinks.filter(link => {
            if (!link.videoId.equals(new mongoose_1.default.Types.ObjectId(videoId))) {
                return false;
            }
            ...
            return link.isPublic || (userId && link.creatorId.equals(new mongoose_1.default.Types.ObjectId(userId)));
        });
        // 開始時間でソート
