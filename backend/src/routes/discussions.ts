...
import { DiscussionThread, DiscussionComment, DiscussionType } from '../models/Discussion';
...
router.post('/threads', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // 必須フィールドの検証
    if (!type || !Object.values(DiscussionType).includes(type)) {
    ...
    // 既存スレッドの確認
    const existingThread = await DiscussionThread.findOne({
    ...
    const thread = new DiscussionThread({
      ...
      type,
      title,
      participantIds: [new mongoose.Types.ObjectId(req.user!.userId)]
    });
...
router.get('/threads', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (shareId) filter.shareId = new mongoose.Types.ObjectId(shareId as string);
    if (evaluationId) filter.evaluationId = new mongoose.Types.ObjectId(evaluationId as string);
    if (sessionId) filter.sessionId = new mongoose.Types.ObjectId(sessionId as string);
    if (type && Object.values(DiscussionType).includes(type as DiscussionType)) {
      filter.type = type;
    }
    ...
    const threadsWithCounts = await Promise.all(
      threads.map(async (thread) => {
        const commentCount = await DiscussionComment.countDocuments({
          threadId: thread._id,
          isDeleted: false
        });
        return {
...
// ディスカッションスレッド詳細取得
router.get('/threads/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
// コメント作成
router.post('/threads/:threadId/comments', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    for (const mention of mentions) {
      if ((mention.userId as mongoose.Types.ObjectId).toString() !== userId) {
        await Notification.createMentionNotification(
          (mention.userId as mongoose.Types.ObjectId).toString(),
          userId,
    ...
    // 作成されたコメントを取得
    const populatedComment = await DiscussionComment.findById(comment._id)
...
// コメント一覧取得
router.get('/threads/:threadId/comments', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { threadId } = req.params;
    ...
    const commentsWithReplies = await Promise.all(
      comments.map(async (comment) => {
        const replyCount = await DiscussionComment.countDocuments({
...
router.put('/comments/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    // 作成者のみ編集可能
    if (!comment.authorId.equals(new mongoose.Types.ObjectId(req.user!.userId))) {
    ...
    const updatedComment = await DiscussionComment.findById(id)
      .populate('authorId', 'username profile.displayName profile.avatar')
      .populate('mentions.userId', 'username profile.displayName');
...
// リアクション追加/削除
router.post('/comments/:id/reactions', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (action === 'add') {
      ...
      if (!comment.authorId.equals(new mongoose.Types.ObjectId(userId))) {
        await Notification.createReactionNotification(
...
// 返信取得
router.get('/comments/:id/replies', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
