...
import { sessionService, SessionListParams } from '../../services/sessionService';
...
const SessionManagement: React.FC<SessionManagementProps> = ({ onSessionSelect }) => {
  ...
  const loadSessions = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'セッション一覧の取得に失敗しました');
  ...
  const getStatusChip = (status: SessionStatus) => {
    const statusConfig = {
      ...
      [SessionStatus.ACTIVE]: { color: 'success' as const, label: 'アクティブ' },
      [SessionStatus.COMPLETED]: { color: 'info' as const, label: '完了' },
      [SessionStatus.ARCHIVED]: { color: 'secondary' as const, label: 'アーカイブ' }
    };
                        ...
                        <Grid item xs={12} md={4}>
                          <Box display="flex" alignItems="center" gap={1}>
                            ...
                            <Typography variant="body2">
                              {(session as any).videoId?.title || '動画情報なし'}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} md={4}>
                          <Box display="flex" alignItems="center" gap={1}>
                            ...
                            <Typography variant="body2">
                              {(session as any).templateId?.name || 'テンプレート情報なし'}
                            </Typography>
                          </Box>
                        </Grid>
                        <Grid item xs={12} md={4}>
                      ...
                      {(session.startDate || session.endDate) && (
                        <Box mt={2}>
                          <Typography variant="caption" color="text.secondary">
                            {session.startDate && `開始: ${new Date(session.startDate).toLocaleString('ja-JP')}`}
                            {session.startDate && session.endDate && ' | '}
                            {session.endDate && `終了: ${new Date(session.endDate).toLocaleString('ja-JP')}`}
                          </Typography>
                        </Box>
                      )}
                    </Box>
        ...
        {selectedSession && (
          <>
            ...
            {selectedSession.status === SessionStatus.DRAFT && (
              <MenuItem
                onClick={() => handleStatusChange(selectedSession, SessionStatus.ACTIVE)}
              >
            ...
            {selectedSession.status === SessionStatus.ACTIVE && (
              <MenuItem
                onClick={() => handleStatusChange(selectedSession, SessionStatus.COMPLETED)}
              >
            ...
            {selectedSession.status === SessionStatus.COMPLETED && (
              <MenuItem
                onClick={() => handleStatusChange(selectedSession, SessionStatus.ARCHIVED)}
              >
